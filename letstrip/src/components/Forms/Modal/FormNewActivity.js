...
export const FormNewActivity = (props) => {
    ...
    console.log(props.rmId)

    const createActivity = (values) => {
        try {
            axios.post(`http://localhost:4000/CreateActivity`,
                values, { headers: { "Content-Type": "application/json" } }).then(res => {
                    if (res) {
                        getActivities(values.day)
                    }
                } 
                )
...
const getActivities = (day) => {
    axios.get(`http://localhost:4000/GetActivities/${props.rmId}/${day}`,
